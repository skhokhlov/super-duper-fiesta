...
- Stage 0 verifies its own success rather than trusting the plugin's exit code. Upstream logs an unresolvable `recipeList` entry at ERROR (`failOnInvalidActiveRecipes` defaults to `false`), runs only the entries it could resolve, and exits 0 with real patches. `DirectPluginExecutor` calls the per-strategy `DirectPluginInvocation.unresolvedRecipeFailure` lambda right after the dry-run — before the diff check and before the apply goal — and returns `PluginRunResult.Failed` naming the unresolved recipes, which routes to `runFullFallback` (the LST classpath is resolved independently and may hold the missing artifact) and keeps a partial migration off disk. **The check runs even when Stage 0 produced diffs**; a `diffs.isEmpty()` guard would miss the defect entirely. Markers live in `PluginOutputReader.unresolvedRecipeFailure`, anchored on exact upstream strings (`recipe '<fqn>' does not exist.`, `Recipe(s) not found: <csv>`, `Recipe validation errors detected as part of one or more activeRecipe(s).`) and therefore version-coupled to the plugin versions pinned in `gradle/libs.versions.toml` — re-verify them on a plugin bump. `No recipes were activated.` is deliberately **not** a marker (it would false-positive on repos that apply the rewrite plugin to subprojects without an `activeRecipe`); that case belongs to #181. See ADR 0006 and issue #268.
