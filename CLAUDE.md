...
- `RecipeLoader.buildAndActivate` fails the load at both ends of the method rather than returning a recipe that is quietly less than what was asked for:
  - **On entry** it probes every recipe classpath entry with `JarFile` before handing it to `ClasspathScanningLoader`, and throws `IllegalArgumentException` naming the path when it is missing, unreadable, or not a readable archive (directories are accepted — upstream scans exploded entries). Aether uses `CHECKSUM_POLICY_IGNORE` deliberately (corporate proxies omit checksums), so a truncated cached JAR is otherwise never detected and upstream's `catch (IOException ignored)` turns it into a misleading "recipe not found". Do not re-add a try/catch around the `ClasspathScanningLoader` constructors — they only store lambdas; the scan runs lazily inside `Environment.activateRecipes()`. Probing first is what keeps a corrupt JAR reported as a corrupt JAR instead of surfacing downstream as an unresolved recipe.
  - **On exit** it rejects a partially initialized recipe. `DeclarativeRecipe.initialize` drops a `recipeList` entry it cannot resolve and only records a `Validated.Invalid` for it, so `RecipeLoader` consults `Recipe.validate()` and throws an `IllegalArgumentException` naming every unresolved recipe. Detection is **structural, not message-based**: failures whose `property` contains `.recipeList[` carry the missing name in `invalidValue` (`getValue()` throws on an `Invalid`). Entries nested inside sub-recipes accumulate onto the root recipe's validation, so one `validate()` call covers the whole list at any depth, and upstream hardcodes `.recipeList` for `preconditions` too, so an unresolved precondition fails the load as well. Only unresolved recipes are fatal — unrelated option validations keep their existing behaviour. Running the subset that did resolve would report success for an incomplete migration (per proposed ADR 0011).
