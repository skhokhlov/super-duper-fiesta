...
- `RecipeLoader` rejects a partially initialized recipe. `DeclarativeRecipe.initialize` drops a `recipeList` entry it cannot resolve and only records `recipe '<fqn>' does not exist.` into its validation state, so `RecipeLoader` consults `Recipe.validate()` and throws an `IllegalArgumentException` naming every unresolved recipe. Entries nested inside sub-recipes accumulate onto the root recipe's validation, so one `validate()` call covers the whole list at any depth. Running the subset that did resolve would report success for an incomplete migration (per proposed ADR 0011).
